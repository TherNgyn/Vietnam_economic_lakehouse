create or replace view gold_staging.stg_ohlc
...
with currency as (
    ...
    from silver.ohlc_currency -- Tên bảng tương ứng bucket s3://silver/ohlc_currency
),
...
idx as (
    select
        ...
        cast(prev_close as decimal(38,10)) as previous_close,
        upper(trim(source)) as source_name
    from silver.ohlc_index -- Tên bảng tương ứng bucket s3://silver/index
),
...
commodity as (
    select
        ...
        cast(prev_close as decimal(38,10)) as previous_close,
        upper(trim(source)) as source_name
    from silver.ohlc_commodity -- Tên bảng tương ứng bucket s3://silver/commodity
),
...
vietnam_index as (
    ...
    from silver.ohlc_vietnam_index -- Tên bảng tương ứng bucket s3://silver/vietnam_index
)
