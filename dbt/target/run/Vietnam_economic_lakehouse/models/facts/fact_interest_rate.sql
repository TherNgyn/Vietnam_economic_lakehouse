        ...
        CREATE TABLE gold_gold.fact_interest_rate
        USING DELTA
        AS
...
joined as (
    select
        ...
        case
            when r.interest_rate is not null and r.volume is not null
                then cast(r.interest_rate as decimal(38,10)) * cast(r.volume as decimal(38,10))
            else null
...
select
    ...
    abs(xxhash64(coalesce(cast(time_key as string), '__null__'), coalesce(cast(asset_key as string), '__null__'), coalesce(cast(term_key as string), '__null__'), coalesce(cast(unit_name as string), '__null__')))
