...
final as (
    ...
    select
        {{ sk(['time_key', 'crop_key', 'source_key']) }} as mart_crop_yield_key,
        ...
        cast(
            round(
                {{ safe_divide('area_value - area_value_pre_year', 'area_value_pre_year') }} * 100,
                3
            ) as decimal(38,3)
        ...
        cast(
            round(
                {{ safe_divide('yield_value - yield_value_pre_year', 'yield_value_pre_year') }} * 100,
                3
            ) as decimal(38,3)
        ...
        cast(
            round(
                {{ safe_divide('productivity_value - productivity_value_pre_year', 'productivity_value_pre_year') }} * 100,
                3
            ) as decimal(38,3)
