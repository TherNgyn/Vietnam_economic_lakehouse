...
with_metrics as (
    ...
    select
        ...
        cast(
            round(
                {{ safe_divide('investment_value - investment_value_pre_quarter', 'investment_value_pre_quarter') }} * 100,
                3
            ) as decimal(38,3)
        ) as qoq_growth_rate,
        ...
        cast(
            round(
                {{ safe_divide('investment_value - investment_value_pre_year', 'investment_value_pre_year') }} * 100,
                3
            ) as decimal(38,3)
        ) as yoy_growth_rate,
...
final as (
    ...
    select
        ...
        cast(
            round(
                {{ safe_divide('investment_value', 'total_investment_value') }} * 100,
                3
            ) as decimal(38,3)
        ) as source_share_pct,
